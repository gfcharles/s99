...
**) Sorting a list of lists according to length of sublists.
    ...
    just once. The third and fourth lists have length 3 and there are two list of this length. Finally, the last three
    lists have length 2. This is the most frequent length.
=end
class Array
  def lsort
    self.sort {|a, b| a.length <=> b.length}
  end

  def lsort_freq
    frequencies = frequency_hash
    self.sort {|a, b| frequencies[a.length] <=> frequencies[b.length]}
  end

  private

  def frequency_hash
    self.reduce({}) do |hash, el|
      hash[el.length] = (hash[el.length] || 0) + 1
      hash
    end
  end
end
...
puts "Result for part A: #{list.lsort}"
puts "Result for part B:  #{list.lsort_freq}"
